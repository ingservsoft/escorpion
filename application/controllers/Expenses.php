...
class Expenses extends Secure_Controller
{
	...
	public function search()
	{
		...
		$saldo=$this->Expense->saldoanterior($filters);
		$data_rows[] = array(
		...
		if($this->Expense->save($expense_data, $expense_id))
		{
			...
			if($expense_id == -1)
			{
				if($tipo_movimiento == 'Egreso' && $expense_category_id == 10){
					...
					$this->Expense_caja->save($expense_data, $expense_id);
				}
	...
	public function receipt_expense($expense_id)
	{
		...
		$data['movimiento']= json_encode($this->Expense->get_info($expense_id));
		$this->load->view('expenses/receipt_expense', $data);
	}

	public function expenses_caja(){
		echo "hola mundoi";
	}
}
?>
