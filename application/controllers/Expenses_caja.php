...
class Expenses_caja extends Secure_Controller
{
	public function __construct()
	{
		parent::__construct('expenses_caja');
	}
	...
	public function search()
	{
		...
		$expenses = $this->Expense_caja->search($search, $filters, $limit, $offset, $sort, $order);
		$total_rows = $this->Expense_caja->get_found_rows($search, $filters);
		$payments = $this->Expense_caja->get_payments_summary($search, $filters);
		$payment_summary = get_expenses_manage_payments_summary($payments, $expenses);		
		$data_rows = array();
		## cambio MARIO LARRAÑAGA que me muestra la columna saldo en el manage de movimientos de caja
		//primero calculamos el saldo anterior
		$saldo=$this->Expense_caja->saldoanterior($filters);
		$data_rows[] = array(
			'description' => '<b>Viene saldo dias anteriores...</b>',
			'saldo'	 => '<b>'. to_currency($saldo).'</b>'
		);
		...
		{
			$arrayfila=get_expenses_data_row($expense,$saldo);
			$data_rows[] = $this->xss_clean($arrayfila);
			$saldo=$this->xss_clean($saldo+$arrayfila['ingreso']-$arrayfila['egreso']);
			$arrayfila['ingreso']=to_currency($arrayfila['ingreso']);
			$arrayfila['egreso']=to_currency($arrayfila['egreso']);
			unset($arrayfila);
		}

		/*if($total_rows>0)
			$data_rows[] = get_expenses_data_last_row($expenses,$saldo);*/
	...
	public function view($expense_id = -1)
	{
		...
		$data['expenses_info'] = $this->Expense_caja->get_info($expense_id);
		...
		$data['payments'] = array();
		foreach($this->Expense_caja->get_expense_payment($expense_id)->result() as $payment)
		{
		...
		$data['payment_options'] = $this->xss_clean($this->Expense_caja->get_payment_options(FALSE));

		$this->load->view("expenses/form_caja", $data);
	}

	public function get_row($row_id)
	{
		$expense_info = $this->Expense_caja->get_info($row_id);
	...
	public function save($expense_id = -1)
	{
		...
		if($this->Expense_caja->save($expense_data, $expense_id))
		{
			...
			if($expense_id == -1)
			{
				if($tipo_movimiento == 'Egreso' && $expense_category_id == 10){
					$expense_data['tipo_movimiento'] = 'Ingreso';
					$this->Expense->save($expense_data, $expense_id);
				}
	...
	public function delete()
	{
		...
		if($this->Expense_caja->delete_list($expenses_to_delete))
		{
	...
	public function receipt_expense($expense_id)
	{
		
		$data['movimiento']= json_encode($this->Expense_caja->get_info($expense_id));
		$this->load->view('expenses/receipt_expense_caja', $data);
	}
