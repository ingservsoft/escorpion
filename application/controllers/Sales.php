...
class Sales extends Secure_Controller
{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($payment_type == $this->lang->line('sales_giftcard')) {
				...
				if (isset($cur_giftcard_customer) && $cur_giftcard_customer != $customer_id) {
					...
					$this->sale_lib->add_payment(-1, $payment_type, $amount_tendered, date("Y-m-d H:i:s"));
				}
	...
	public function add()
	{
		...
		if ($mode == 'return' && $this->Sale->is_valid_receipt($item_id_or_number_or_item_kit_or_receipt)) {
			...
			## cambio MARIO LARRAÑAGA para no dejar agregar productos sin stock
			$item_info = $this->Item->get_info_by_id_or_number($item_id_or_number_or_item_kit_or_receipt, FALSE);
			$item_id = $item_info ? $item_info->item_id : -1;
			$data['error'] = $this->sale_lib->out_of_stock($item_id, $item_location);
			if ($data['error'] != '' && $this->config->item('ventas_sin_stock') != '1') {
				$this->sale_lib->delete_item($item_id_or_number_or_item_kit_or_receipt);
	...
	public function complete()
	{
		...
		// Devuelve 'subtotal', 'total', 'total en efectivo', 'total de pago', 'monto adeudado', 'monto en efectivo adeudado', 'total de cobertura de pagos'
		...
		if ($data['amount_change'] > 0) {
			...
			foreach ($data['payments'] as $id => $payment) {
				if ($payment['payment_type'] == $this->lang->line('sales_cash')) {
					$data['payments'][$id]['cash_refund'] = $data['amount_change'];
					break;
				}
			}
		}
		...
		if ($this->sale_lib->is_invoice_mode())  //MODO FACTURA
		{
		...
		} elseif ($this->sale_lib->is_work_order_mode())  // MODO ORDEN DE TRABAJO O PEDIDO
		{
		...
		} elseif ($this->sale_lib->is_quote_mode()) // MODO COTIZACION
		{
		...
		} else  // MODO POS
		{
			...
			if ($data['sale_id_num'] == -1)
				$data['error_message'] = $this->lang->line('sales_transaction_failed');
			// cambio MARIO LARRAÑAGA para guardar el movimeinto de ingreso POS
			elseif ($this->guardaMovimientoIngreso($data['cart'], $data['payments'], $customer_id, $customer_info->tax_id, $data['sale_id_num'], $employee_id, $totals)) {
				$expense_data = $this->xss_clean($expense_data);
				## CAMBIO MARIO LARRAÑAGA - cambiamos la disponibilidad a FACTURADA
				$this->Disponibilidad->set_estado_disponibilidad_by_sale($sale_id, "FACTURADA");
			} else {
				$data['error_message'] = "Hubo un error al guardar el movimiento en caja<br />" . json_encode($expense_data);
			}
	...
	public function edit($sale_id, $saldo = 0)
	{
		$data = array();
	...
	public function cancel()
	{
		...
		if ($sale_id != -1 && $sale_id != '') {
			...
			if ($sale_type == SALE_TYPE_WORK_ORDER) {
				...
				##cambio MARIO LARRAÑAGA si hay disponibilidad debemos ponerla disponible
				$this->Disponibilidad->set_estado_disponibilidad_by_sale($sale_id, "DISPONIBLE");
	...
	{
		$selected = $this->input->post('sales');
		$sales = json_decode($selected);
		foreach ($sales as $item) {
			$sale_id = explode("-", $item)[0];
			$sale_type = explode("-", $item)[1];
			if ($sale_id != -1 && $sale_id != '') {
				if ($sale_type == SALE_TYPE_WORK_ORDER) {
					$this->Sale->update_sale_status($sale_id, CANCELED);
				} else {
	...
	public function suspend()
	{
		...
		$sale_id = $this->sale_lib->get_sale_id();
		$sale_info = $this->Sale->get_info_sale($sale_id);
		$dinner_table = $this->sale_lib->get_dinner_table();
		...
		if ($sale_info->sale_status == COMPLETED) {
			$this->Disponibilidad->set_estado_disponibilidad_by_sale($sale_id, 'RESERVADA');
		}
		...
		if ($this->Sale->save($sale_id, $sale_status, $cart, $customer_id, $employee_id, $comment, $invoice_number, $work_order_number, $quote_number, $sale_type, $payments, $dinner_table, $sales_taxes) == '-1') {
			...
			$this->guardaMovimientoIngreso($cart, $payments, $customer_id, NULL, $sale_id, $employee_id);
	...
	/**
	 * List suspended sales customer(CarlosCh)
	 */
	public function suspendedByCustomer($customer_id, $sale_id, $abono, $accion, $estado = 1)
	{
		$data = array();
		//aqui valido si hay reservas contradas par cliente (vienen en order desacendente)
		$disponibilidad = $this->Disponibilidad->getDisponibilidadBySaleId($sale_id);
		$reservas_contratadas = $this->Sale->get_reservas_contratadas($customer_id,$disponibilidad->contract_id);
		if (count($reservas_contratadas) > 0) {
			$last_reserva_contratada = $reservas_contratadas[0];
			$data["last_reserva_contratada"] = $last_reserva_contratada;
		}
		$data['payments'] = $this->sale_lib->get_payments();
		$data['sale_old'] = $sale_id;
		$data['abono'] = $abono;
		$data['suspended_sales'] = $this->xss_clean($this->Sale->get_all_suspended_excepto_sale_id($customer_id, $sale_id));
		$data['accion'] = $accion;
		$data['estado_sale'] = $estado;
		$this->load->view('sales/formabono', $data);
	}
	...
	function unsuspend_retomada($sale_id, $payment_type, $payment_amount, $fechapago, $estado_sale)
	{
		$this->sale_lib->clear_all();
		if ($sale_id > 0) {
			$this->sale_lib->copy_entire_sale($sale_id);
			...
			$this->sale_lib->add_payment(-1, $payment_type, $payment_amount, $fecha);
		}

		// Set current register mode to reflect that of unsuspended order type
		$this->change_register_mode($this->sale_lib->get_sale_type());
		$data['estado_sale'] = $estado_sale;
		$this->_reload($data);
	}
	...
	## cambio MARIO LARRAÑAGA - guardamos el movimiento de caja POS
	private function guardaMovimientoIngreso($carrito, $pagos, $customer_id, $custommer_tax_id, $sale_id, $employee_id, $totals = null)
	{
		$resultado = true;

		//primero eliminamos los movimientos de pago de sale_id
		if ($sale_id != -1 && $sale_id != NULL)
			$this->db->query("DELETE FROM " . $this->db->dbprefix('expenses') . " WHERE sale_id='" . $sale_id . "'");

		$txt_carrito = '';
		$category_id = 0;
		$totalcarrito = 0;
		foreach ($carrito as $item) {
			if (strpos("Alquiler de Cancha", $item['name']) != false) //solo para articulos diferentes a canchas
			{
				$txt_carrito .= '[' . round($item['quantity'], 0) . ']' . $item['name'] . ' ' . $item['description'] . ' - ';
				$category_id = 1;
			} else
				$txt_carrito .= $item['name'] . ' ' . $item['description'] . ' - ';


			$totalcarrito += $item['total'];
		}
		$txt_carrito = rtrim($txt_carrito, ' - ');
		//start calculo del ingreso (CarlosCh)
		if (isset($totals)) {
			$suma_bonos = 0;
			$cont = 1;
			foreach ($pagos as $pay) {
				...
				if (count($pagos) != $cont) {
					$suma_bonos =  $suma_bonos + $payment_amount;
				}
				$cont++;
			}
		}
		...
		foreach ($pagos as $payment) {
			if ($payment['payment_type'] == 'Efectivo' && $payment['payment_amount'] != 0) {
				if (isset($totals) && $totals['amount_due'] != 0) {
					if (count($pagos) == $cont) {
						$saldo = $totals['total'] - $suma_bonos;
					} else {
						$saldo = $payment['payment_amount'];
					}
				} else {
					$saldo = $payment['payment_amount'];
				}
				if ($totalcarrito - $saldo > 0)
					$txt_carrito = 'Abono: ' . $txt_carrito;
				$expense_data = array(
					'date' => $payment['payment_time'],
					'supplier_id' => $customer_id > 0 ? $customer_id : NULL,
					'supplier_tax_code' => $custommer_tax_id,
					'amount' => $saldo,
					'tax_amount' => parse_decimals(0),
					'payment_type' => 'Efectivo',
					'expense_category_id' => 1,
					'description' => $txt_carrito,
					'employee_id' => $employee_id,
					'deleted' => 0,
					'tipo_movimiento' => 'Ingreso',
					'caja' => 'Caja POS',
					'sale_id' => $sale_id != -1 ? $sale_id : NULL,
				);
				$resultado = $this->Expense->save($expense_data, -1);
			}
			$cont++;
		}
		return $resultado;
	}
	## fin cambio MARIO LARRAÑAGA
	##cambio carlos chaucanes
	public function getSuspendedCustomer($customer_id)
	{
		$res = $this->xss_clean($this->Sale->get_all_suspended($customer_id));
		echo json_encode($res);
	}

	public function getSuspendedCustomerExceptoSaleId($customer_sale_id)
	{
		$customer_id = explode("_", $customer_sale_id)[0];
		$sale_id = explode("_", $customer_sale_id)[1];
		$res = $this->xss_clean($this->Sale->get_all_suspended_excepto_sale_id($customer_id, $sale_id));
		echo json_encode($res);
	}
	public function updateAbonoNextSale()
	{
		$sale_old = $this->input->get('sale_old');
		$sale_new = $this->input->get('sale_new');
		$cambio = $this->input->get('cambio');
		$sale_info = $this->Sale->get_info($sale_new)->row();
		$customer_id = $sale_info->customer_id;
		$success = $this->Sale->updateAbonoPayment($sale_old, $sale_new, $cambio, $this->session->userdata('person_id'));
		$sql = $this->db->last_query();
		## agregar el pago a expenses como un ingreso con los detalles de la reserva siguiente ##
		$person = $this->Customer->get_info($customer_id);
		$items_sale_new =  $this->Sale->get_sale_items($sale_new)->result_array();

		$expense_data = array(
			'date' => date("Y-m-d H:i:s"),
			'supplier_id' => $customer_id > 0 ? $customer_id : NULL,
			'supplier_tax_code' => $person->tax_id,
			'amount' => $cambio,
			'tax_amount' => parse_decimals(0),
			'payment_type' => 'Efectivo',
			'expense_category_id' => 1,
			'description' => "Abono: " . $items_sale_new[0]['name'] . ' ' . $items_sale_new[0]['description'],
			'employee_id' => $this->session->userdata('person_id'),
			'deleted' => 0,
			'tipo_movimiento' => 'Ingreso',
			'caja' => 'Caja POS',
			'sale_id' => $sale_new != -1 ? $sale_new : NULL,
		);
		$resultado = $this->Expense->save($expense_data, -1);
	...
	public function updateAbononextSaleCancel()
	{
		$sale_old = $this->input->get('sale_old');
		$sale_new = $this->input->get('sale_new');
		$cambio = $this->input->get('cambio');
	...
	public function move_abono()
	{
		$sale_old = $this->input->post('sale_old');
		$sale_new = $this->input->post('sale_new');
		$abono = $this->input->post('abono');
		$accion = $this->input->post('accion');
		$customer_id = $this->input->post('customer_id');
		$estado_sale = $this->input->post('estado_sale');
		$new_disponibilidad = $this->input->post('new_disponibilidad');
		if ($accion == 'completar_venta') {
			$success = $this->Sale->insertAbonoPaymentSaleId($sale_old, $sale_new, $abono, $this->session->userdata('person_id'), $customer_id);
			if ($new_disponibilidad != 0) {
				$dis = $this->Disponibilidad->getDisponibilidadBySaleId($sale_old);
				$res = $this->Disponibilidad->do_reserva($customer_id, $new_disponibilidad, $dis->contract_id);
				//$this->reservarNext($customer_id,$new_disponibilidad,$accion);
			}
		} else {
			$success = $this->Sale->updateAbonoPaymentSaleId($sale_old, $sale_new, $abono, $this->session->userdata('person_id'), $estado_sale);
		}

		echo $success;
	}

	public function reservarNext($customer_id, $sale_old,$accion)
	{
		$data = array();
		$dis = $this->Disponibilidad->getDisponibilidadBySaleId($sale_old);

		$reservas_contratadas = $this->Sale->get_reservas_contratadas($customer_id,$dis->contract_id);
		if (count($reservas_contratadas) > 0) {
			$last_reserva_contratada = $reservas_contratadas[0];
			$data["last_reserva_contratada"] = $last_reserva_contratada;
		}
		$data['sale_old']=$sale_old;
		$data['accion'] = $accion;
		$data['customer_id'] = $customer_id;
		$this->load->view('sales/confirm_reserva', $data);
	}
	public function renewReserva()
	{
		$sale_id = $this->input->post('sale_id');
		$dis = $this->Disponibilidad->getDisponibilidadBySaleId($sale_id);
		$customer_id = $this->input->post('person_id');
		$new_disponibilidad = $this->input->post('disponibilidad_new');
		$res = $this->Disponibilidad->do_reserva($customer_id, $new_disponibilidad,$dis->contract_id);
		echo $res;
	}
}
