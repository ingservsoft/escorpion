...
class Items extends Secure_Controller
{
	...
	//Save item data
		$item_data = array(
			...
			'item_codigolocal' => $this->input->post('item_codigolocal') == '' ? NULL : $this->input->post('item_codigolocal'),
	...
	public function check_item_codigolocal()
	{
		$exists = $this->Item->item_codigolocal_exists($this->input->post('item_codigolocal'), $this->input->post('item_id'));
		echo !$exists ? 'true' : 'false';
	}
	/*
	...
	public function save_inventory($item_id = -1)
	{
		...
		$trans_date=$this->input->post('trans_date'); //cambio MARIO LARRAÑAGA para la fecha del ajuste
		$date_formatter = date_create_from_format($this->config->item('dateformat') . ' ' . $this->config->item('timeformat'), $trans_date);
		$inv_data = array(
			'trans_date' => $date_formatter->format('Y-m-d H:i:s'), //aqui cambia el anterior era date('Y-m-d H:i:s')
			'trans_items' => $item_id,
			'trans_user' => $employee_id,
