...
class Reports extends Secure_Controller
{
	...
	public function summary_sales($start_date, $end_date, $sale_type, $location_id = 'all', $sale_status = 0)
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type, 'location_id' => $location_id, 'sale_status' => $sale_status);
	...
	public function detailed_sales($start_date, $end_date, $sale_type, $location_id = 'all', $sale_status = 0)
	{
		$definition_names = $this->Attribute->get_definitions_by_flags(Attribute::SHOW_IN_SALES);

		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type, 'location_id' => $location_id, 'sale_status' => $sale_status, 'definition_ids' => array_keys($definition_names));
	...
	## Reportes agregados por MARIO LARRAÑAGA 
	//Summary dinner_table report
	public function summary_dinnertables($start_date, $end_date)
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => 'complete', 'location_id' => 'all');

		$this->load->model('reports/Summary_dinnertable');
		$model = $this->Summary_dinnertable;
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = $this->xss_clean(array(
				...
				'total_venta' => to_currency($row['total_venta']),
				'efectivo' => to_currency($row['efectivo']),
				'tarjeta_debito' => to_currency($row['tarjeta_debito']),
				'tarjeta_credito' => to_currency($row['tarjeta_credito']),
				'credito' => to_currency($row['credito']),
				'tarjeta_regalo' => to_currency($row['tarjeta_regalo'])
			));
		}

		$data = array(
			'title' => $this->lang->line('reports_dinnertable_summary_report'),
			'subtitle' => $this->_get_subtitle_report(array('start_date' => $start_date, 'end_date' => $end_date)),
			'headers' => $this->xss_clean($model->getDataColumns()),
			'data' => $tabular_data,
			'summary_data' => $summary
	...
	public function inventory_kardex($start_date, $end_date)
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => 'complete', 'location_id' => 'all');

		$this->load->model('reports/Inventory_kardex');
		$model = $this->Inventory_kardex;
		...
		foreach($report_data as $row)
		{
			$stockfin=$row['stock_ini']+$row['entradas']-$row['salidas']+$row['ajuste'];
			$tabular_data[] = $this->xss_clean(array(
				'item_codigolocal' => $row['item_codigolocal'],
				'item_name' => $row['name'],
				'item_number' => $row['item_number'],
				'category' => $row['category'],
				'stock_ini' => to_quantity_decimals($row['stock_ini']),
				'entradas' => to_quantity_decimals($row['entradas']),
				'salidas' => to_quantity_decimals($row['salidas']),
				'ajuste' => $row['ajuste']?to_quantity_decimals($row['ajuste']):'',
				'stock_fin' => to_quantity_decimals($stockfin)
			));
		}

		$data = array(
			'title' => $this->lang->line('reports_inventory_kardex_report'),
			'subtitle' => $this->_get_subtitle_report(array('start_date' => $start_date, 'end_date' => $end_date)),
			'headers' => $this->xss_clean($model->getDataColumns()),
			'data' => $tabular_data,
			'summary_data' => ''
