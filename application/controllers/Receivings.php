...
class Receivings extends Secure_Controller
{
	...
	public function manage()
	{
		...
		{
			$data['table_headers'] = transform_headers(array(
				array('receiving_id' => "ID"),
				array('receiving_time' => "Fecha"),
				array('customer_name' => "Proveedor"),
				array('amount_due' => "Monto Adeudado"),
				array('amount_tendered' => "Monto Pagado"),
				array('change_due' => "Saldo"),
				array('payment_type' => "Tipo de Pago"),
	...
	/*CAMBIOS MARIO LARRAÑAGA para Cuentas por Pagar suspend*/
	public function suspend()
	{
		$receiving_id = $this->receiving_lib->get_receiving_id(); // cerar en la lib
		...
		$show_stock_locations = $this->Stock_location->show_locations('receivings');
		$stock_location = $this->receiving_lib->get_stock_source();
		if($this->input->post('amount_tendered') != NULL)
		{
			$amount_tendered = $this->input->post('amount_tendered');
			$amount_change = to_currency($amount_tendered - $total);
		}
		...
		if($supplier_id != -1)
		{
			...
			$supplier_address = $supplier_info->address_1;
			if(!empty($supplier_info->zip) or !empty($supplier_info->city))
			{
				$supplier_location = $supplier_info->zip . ' ' . $supplier_info->city;				
			}
			else
			{
				$supplier_location = '';
		...
		$receivings_status = SUSPENDED;

		if($this->Receiving->save($cart, $supplier_id, $employee_id, $comment, $reference, $payment_type, $stock_location, $receivings_status) == '-1')
		{
			$data['error'] = "Error al guardar la compra por pagar";
		}
		else
		{
			$data['success'] = "La compra por pagar se guardó satisfactoriamente";
		}
	...
	// lista de compras suspendidas
	public function suspended()
	{
		$data = array();
		$supplier_id = $this->receiving_lib->get_supplier();
		$data['suspended_receivings'] = $this->xss_clean($this->Receiving->get_all_suspended($supplier_id));
