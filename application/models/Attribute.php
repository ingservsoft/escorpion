	...
	//From TEXT
		...
		//To DATETIME or DECIMAL
			if(in_array($to_type, [DATE, DECIMAL], TRUE))
			{
				...
				if($this->check_data_validity($definition_id, $from_type, $to_type))
				{
					...
					$query = 'UPDATE ' . $this->db->dbprefix('attribute_values');
					$query .= 'INNER JOIN '. $this->db->dbprefix('attribute_links');
					$query .= 'ON '. $this->db->dbprefix('attribute_values').'.attribute_id = '. $this->db->dbprefix('attribute_links').'.attribute_id ';
		...
		//To DROPDOWN or CHECKBOX
			...
			else if($to_type === CHECKBOX)
			{
				...
				$query = 'UPDATE '. $this->db->dbprefix('attribute_values').' values ';
				$query .= 'INNER JOIN '. $this->db->dbprefix('attribute_links').' links ';
		...
		else if($from_type === DROPDOWN)
		{
			...
			if(in_array($to_type, [TEXT, CHECKBOX], TRUE))
			{
				...
				if($to_type === CHECKBOX)
				{
					...
					$query = 'UPDATE '. $this->db->dbprefix('attribute_values').' vals ';
					$query .= 'INNER JOIN '. $this->db->dbprefix('attribute_links').' links ';
