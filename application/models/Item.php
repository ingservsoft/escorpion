...
class Item extends CI_Model
{
	...
	*/
	...
	public function item_codigolocal_exists($item_codigolocal, $item_id = '')
	{
		if($this->config->item('allow_duplicate_barcodes') != FALSE)
		{
			return FALSE;
		...
		$this->db->from('items');
		$this->db->where('item_codigolocal', (string) $item_codigolocal);
		// check if $item_id is a number and not a string starting with 0
		// because cases like 00012345 will be seen as a number where it is a barcode
		if(ctype_digit($item_id) && substr($item_id, 0, 1) != '0')
		{
	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc', $count_only = FALSE)
	{
		...
		else
		{
			...
			$this->db->select('MAX(items.item_codigolocal) AS item_codigolocal');
		...
		if(!empty($search))
		{
			...
			else
			{
				$this->db->group_start();
					...
					$this->db->or_like('item_codigolocal', $search);
	...
	*/
	...
	public function update_item_codigolocal($item_id, $item_codigolocal)
	{
		$this->db->where('item_id', $item_id);
		$this->db->update('items', array('item_codigolocal'=>$item_codigolocal));
	}
