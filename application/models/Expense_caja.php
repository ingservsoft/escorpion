...
class Expense_caja extends CI_Model
{
	/*
	...
	*/
	...
	{
		$this->db->from('expenses_caja');
		$this->db->where_in('expenses_caja.expense_id', $expense_ids);
	...
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'expense_id', $order = 'asc', $count_only = FALSE)
	{
		...
		else
		{
			$this->db->select('
				...
				MAX(CONCAT(people.first_name, " ", people.last_name) ) AS supplier_name,
				...
				MAX(expenses.tipo_movimiento) AS tipo_movimiento,
				MAX(expenses.caja) AS caja
			');
		}

		$this->db->from('expenses_caja AS expenses');
		...
		$this->db->join('people AS people', 'people.person_id = expenses.supplier_id', 'LEFT');
		...
		$this->db->group_start();
			...
			$this->db->or_like('expenses.caja', $search);
		$this->db->group_end();
	...
	*/
	public function get_info($expense_id)
	{
		$this->db->select('
			...
			CONCAT(people.first_name," ",people.last_name) AS supplier_name,
			...
			expenses.tipo_movimiento AS tipo_movimiento,
			expenses.caja AS caja
		');
		$this->db->from('expenses_caja AS expenses');
		$this->db->join('people AS employees', 'employees.person_id = expenses.employee_id', 'LEFT');
		$this->db->join('expense_categories AS expense_categories', 'expense_categories.expense_category_id = expenses.expense_category_id', 'LEFT');
		...
		else
		{
			...
			foreach($this->db->list_fields('expenses_caja') as $field)
			{
				$expenses_obj->$field = '';
			}
	...
	*/
	public function save(&$expense_data, $expense_id = FALSE)
	{
		...
		{
			if($this->db->insert('expenses_caja', $expense_data))
			{
		...
		return $this->db->update('expenses_caja', $expense_data);
	}
	...
	*/
	public function delete_list($expense_ids)
	{
		...
		$this->db->trans_start();
			...
			$success = $this->db->update('expenses_caja', array('deleted'=>1));
	...
	*/
	...
	public function saldoanterior($filters)
	{
		$sql='SELECT (
		(SELECT IFNULL(SUM(amount),0) FROM '. $this->db->dbprefix('expenses_caja')			
			. ' WHERE DATE_FORMAT(date, "%Y-%m-%d") < ' . $this->db->escape($filters['start_date']) 
			. ' AND tipo_movimiento="Ingreso" AND deleted = 0) 
		-
		(SELECT IFNULL(SUM(amount),0) FROM '. $this->db->dbprefix('expenses_caja')			
			. ' WHERE DATE_FORMAT(date, "%Y-%m-%d") < ' . $this->db->escape($filters['start_date']) 
			. ' AND tipo_movimiento="Egreso" AND deleted = 0) 
		) AS saldo';
