...
class Sale extends CI_Model
{
	 ...
	 * Get sale info
	 */
	public function get_info_sale($sale_id){
		$this->db->from('sales');
	...
	{
		//echo "ID:".$sale_id."<hr />Status: ".$sale_status."<hr />".json_encode($items)."<hr />".$customer_id."<hr />".$employee_id."<hr />".$comment."<hr />".$invoice_number."<hr />".$work_order_number."<hr />".$quote_number."<hr />".$sale_type."<hr />".json_encode($payments)."<hr />".$dinner_table."<hr />".json_encode($sales_taxes);
		if($sale_id != -1)
		{
		...
		foreach($payments as $payment_id=>$payment)
		{
			...
			$sales_payments_data = array(
				...
				'payment_type'	 => $payment['payment_type'],
				'payment_amount' => $payment['payment_amount'],
				//'cash_refund'    => $payment['cash_refund'], //asi estaba antes de multiples pagos por fecha
				'cash_refund'    => 0,
				'payment_time'    => $payment['payment_time'], //esta es nueva con el cambio
				'employee_id'	 => $employee_id
			);

			$this->db->insert('sales_payments', $sales_payments_data);
			//echo "<hr />".$this->db->last_query();
			//echo "<hr /> - ".$this->db->insert_id();
	...
	{
		$this->db->select('sales_items.*, items.name');
	...
	public function get_all_suspended($customer_id = NULL)
	{
		...
		if($customer_id == -1)
		{
			$sql = "SELECT sa.sale_id, case when sa.sale_type = '".SALE_TYPE_QUOTE."' THEN sa.quote_number WHEN sa.sale_type = '".SALE_TYPE_WORK_ORDER."' THEN sa.work_order_number else sa.sale_id end as doc_id, sa.sale_id as suspended_sale_id, sa.sale_status, sa.sale_time, sa.dinner_table_id, sa.customer_id, sa.employee_id, di.fechahora_inicio as date_reservation,sa.sale_type as tipo,ca.nombre_cancha as cancha, comment FROM "
				. $this->db->dbprefix('sales') . ' sa inner join '.$this->db->dbprefix('disponibilidad').' di on sa.sale_id = di.sale_id inner join '.$this->db->dbprefix('canchas').' ca on ca.id_cancha = di.id_cancha where sale_status = ' . SUSPENDED.' order by date_reservation ASC';

		}
		else
		{
			$sql = "SELECT sa.sale_id, case when sa.sale_type = '".SALE_TYPE_QUOTE."' THEN sa.quote_number WHEN sa.sale_type = '".SALE_TYPE_WORK_ORDER."' THEN sa.work_order_number else sa.sale_id end as doc_id, sa.sale_status, sa.sale_time, sa.dinner_table_id, sa.customer_id, sa.employee_id, di.fechahora_inicio as date_reservation, sa.sale_type as tipo,ca.nombre_cancha as cancha, comment FROM "
				. $this->db->dbprefix('sales') . ' sa inner join '.$this->db->dbprefix('disponibilidad').' di on sa.sale_id = di.sale_id  inner join '.$this->db->dbprefix('canchas').' ca on ca.id_cancha = di.id_cancha where sale_status = '. SUSPENDED .' AND customer_id = ' . $customer_id.' order by date_reservation ASC';
    ...
    //funcion para obtemer las reservas excepto la actual (CarlosCh)
	public function get_all_suspended_excepto_sale_id($customer_id,$sale_id){
		$sql = "SELECT sa.sale_id, case when sa.sale_type = '".SALE_TYPE_QUOTE."' THEN sa.quote_number WHEN sa.sale_type = '".SALE_TYPE_WORK_ORDER."' THEN sa.work_order_number else sa.sale_id end as doc_id, sa.sale_status, sa.sale_time, sa.dinner_table_id, sa.customer_id, sa.employee_id, di.fechahora_inicio as date_reservation, sa.sale_type as tipo,ca.nombre_cancha as cancha, comment FROM "
		. $this->db->dbprefix('sales') . ' sa inner join '.$this->db->dbprefix('disponibilidad').' di on sa.sale_id = di.sale_id  inner join '.$this->db->dbprefix('canchas').' ca on ca.id_cancha = di.id_cancha where   sale_status = '. SUSPENDED .' AND customer_id = ' . $customer_id .' AND sa.sale_id <> '.$sale_id.' order by date_reservation ASC';
		$query = $this->db->query($sql);
		return $query->result_array();
	}

	//funcion para para obtener la reserva actual
	public function getCurrentReserva($customer_id,$sale_id){
		$sql = "SELECT sa.sale_id, case when sa.sale_type = '".SALE_TYPE_QUOTE."' THEN sa.quote_number WHEN sa.sale_type = '".SALE_TYPE_WORK_ORDER."' THEN sa.work_order_number else sa.sale_id end as doc_id, sa.sale_status, sa.sale_time, sa.dinner_table_id, sa.customer_id, sa.employee_id, di.fechahora_inicio as date_reservation, sa.sale_type as tipo,ca.nombre_cancha as cancha, comment FROM "
		. $this->db->dbprefix('sales') . ' sa inner join '.$this->db->dbprefix('disponibilidad').' di on sa.sale_id = di.sale_id  inner join '.$this->db->dbprefix('canchas').' ca on ca.id_cancha = di.id_cancha where   sale_status = '. SUSPENDED .' AND customer_id = ' . $customer_id .' AND sa.sale_id = '.$sale_id.' order by date_reservation ASC';
		$query = $this->db->query($sql);
		return $query->result_array();
	...
	public function updateAbonoPayment($sale_old,$sale_new,$cambio,$employee_id){
		/*$sales_payments_data = array(
			'sale_id' => $sale_new,
			'payment_amount'=>$cambio
		);
		$this->db->where('sale_id', $sale_old);
		$success = $this->db->update('sales_payments', $sales_payments_data);*/
			// Add a new payment transaction
			$sales_payments_data = array(
				'sale_id' => $sale_new,
	...
	public function getTotPaymentsSale($sale_id)
	{
		if($sale_id)
		{
			$sql="SELECT SUM(payment_amount) AS abono FROM ".$this->db->dbprefix('sales_payments')." WHERE sale_id=".$sale_id;
			$result=$this->db->query($sql);
			return $result->row()->abono;
		}
		else
		{
			return 0;
	...
	public function updateAbonoPaymentSaleId($sale_old,$sale_new,$cambio,$employee_id,$estado_sale=1){
		$this->db->trans_start();
			// Add a new payment transaction
			$success = false;
			$sales_payments_data = array(
			...
			if($estado_sale==0){
				$sales_payments_data['payment_amount'] = $cambio;
			}

			$this->db->where('sale_id',$sale_old);
			$this->db->update('sales_payments', $sales_payments_data);

			$sales_expenses_data = array(
				'sale_id' => $sale_new,
				'employee_id' => $employee_id,
			);
			if($estado_sale==0){
				$sales_expenses_data['amount'] = $cambio;
			}

			$this->db->where('sale_id',$sale_old);
			$this->db->update('expenses', $sales_expenses_data);
	...
	public function insertAbonoPaymentSaleId($sale_old,$sale_new,$cambio,$employee_id,$customer_id){
		$this->db->trans_start();
			// Add a new payment transaction
			$success = false;
			$sales_payments_data = array(
			...
			$items_sale_new =  $this->get_sale_items($sale_new)->result_array();
			...
			$this->db->insert('expenses', $sales_payments_data);
	...
	public function get_reservas_contratadas($customer_id,$contract_id=-1){

		$sql="
			select * from qc_disponibilidad dis join qc_sales sal on dis.sale_id = sal.sale_id
			where sal.customer_id = ".$customer_id." and dis.estado = 'RESERVADA' and contract_id = ".$contract_id." 
