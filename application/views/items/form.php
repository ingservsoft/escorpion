...
<?php echo form_open('items/save/'.$item_info->item_id, array('id'=>'item_form', 'enctype'=>'multipart/form-data', 'class'=>'form-horizontal')); ?>
	<fieldset id="item_basic_info">
        ...
        <div class="form-group form-group-sm">
			<?php echo form_label($this->lang->line('items_item_codigolocal'), 'item_codigolocal', array('class'=>'control-label col-xs-3')); ?>
			<div class='col-xs-8'>				
				<?php echo form_input(array(
                        'name'=>'item_codigolocal',
                        'id'=>'item_codigolocal',
                        'class'=>'form-control input-sm',
                        'value'=>$item_info->item_codigolocal)
                        );?>				
			</div>
		</div>
		...
		$('#item_form').validate($.extend({
			...
			rules:
			{
				...
				item_codigolocal:
				{
					...
					remote:
					{
						url: "<?php echo site_url($controller_name . '/check_item_codigolocal')?>",
						type: 'POST',
						data: {
							'item_id' : "<?php echo $item_info->item_id; ?>",
							'item_codigolocal' : function()
							{
								return $('#item_codigolocal').val();
							},
						}
					}
				},
			...
			messages:
			{
				...
				item_codigolocal: "<?php echo $this->lang->line('items_item_codigolocal_duplicate'); ?>",
