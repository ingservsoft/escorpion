	...
	$('#customer_form').validate($.extend({
		submitHandler: function(form) {
			$(form).ajaxSubmit({
				success: function(response)
				{
					...
					dialog_support.hide();
					if('<?php echo $origin;?>' == 'customer'){
						table_support.handle_submit("<?php echo site_url($controller_name); ?>", response);
					}else if('<?php echo $origin;?>' == 'availability'){
						$("#person_id").val(datos.id);
						$("#person_name").val(datos.name);
		...
		rules:
		{
			...
			phone_number:
			{
				...
				remote:
				{
					url: "<?php echo site_url($controller_name . '/ajax_check_phone_number') ?>",
					type: 'POST',
					data: {
						'person_id': "<?php echo $person_info->person_id; ?>"
						// email is posted by default
		...
		messages:
		{
			...
			phone_number:
			{
				required:"<?php echo $this->lang->line('common_phone_number_required');?>",
				remote:"<?php echo $this->lang->line('common_phone_number_duplicate');?>",
			}
		}
	}, form_support.error));
});
