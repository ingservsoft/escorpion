...
UPDATE qc_sales t1
  SET sale_type = 4
WHERE EXISTS (SELECT t2.sale_id FROM qc_sales_items t2 WHERE t1.sale_id = t2.sale_id AND t2.quantity_purchased < 0);
...
DELETE FROM `qc_modules` WHERE `qc_modules`.`module_id` = 'migrate';
DELETE FROM `qc_permissions` WHERE `qc_permissions`.`permission_id` = 'migrate';
DELETE FROM `qc_grants` WHERE `qc_grants`.`permission_id` = 'migrate' AND `qc_grants`.`person_id` = 1;
...
-- Table structure for table `qc_expense_categories`

CREATE TABLE IF NOT EXISTS `qc_expense_categories` (
...
-- Table structure for table `qc_expenses`

CREATE TABLE IF NOT EXISTS `qc_expenses` (
...
-- Indexes for table `qc_expense_categories`

ALTER TABLE `qc_expense_categories`
  ADD PRIMARY KEY (`expense_category_id`),
  ADD UNIQUE KEY `category_name` (`category_name`);


-- Indexes for table `qc_expenses`

ALTER TABLE `qc_expenses`
  ADD PRIMARY KEY (`expense_id`),
  ADD KEY `expense_category_id` (`expense_category_id`),
  ADD KEY `employee_id` (`employee_id`);


-- AUTO_INCREMENT for table `qc_expense_categories`

ALTER TABLE `qc_expense_categories`
  MODIFY `expense_category_id` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;


-- AUTO_INCREMENT for table `qc_expenses`

ALTER TABLE `qc_expenses`
  MODIFY `expense_id` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;


-- Constraints for table `qc_expenses`

ALTER TABLE `qc_expenses`
  ADD CONSTRAINT `qc_expenses_ibfk_1` FOREIGN KEY (`expense_category_id`) REFERENCES `qc_expense_categories` (`expense_category_id`),
  ADD CONSTRAINT `qc_expenses_ibfk_2` FOREIGN KEY (`employee_id`) REFERENCES `qc_employees` (`person_id`);


-- Add extra columns to `qc_expenses`
...
DELETE FROM `qc_app_config` WHERE `key` = 'barcode_quality';
