...
ALTER TABLE `qc_receivings_items`
	CHANGE COLUMN `discount_percent` `discount` DECIMAL(15,2) NOT NULL DEFAULT 0 AFTER `item_unit_price`,
	ADD COLUMN `discount_type` TINYINT(2) NOT NULL DEFAULT '0' AFTER `discount`;
...
-- Table structure for table `qc_cash_up`

CREATE TABLE `qc_cash_up` (
...
-- Indexes for table `qc_cash_up`

ALTER TABLE `qc_cash_up`
  ADD PRIMARY KEY (`cashup_id`),
  ADD KEY `open_employee_id` (`open_employee_id`),
  ADD KEY `close_employee_id` (`close_employee_id`),
  ADD CONSTRAINT `qc_cash_up_ibfk_1` FOREIGN KEY (`open_employee_id`) REFERENCES `qc_employees` (`person_id`),
  ADD CONSTRAINT `qc_cash_up_ibfk_2` FOREIGN KEY (`close_employee_id`) REFERENCES `qc_employees` (`person_id`);

-- AUTO_INCREMENT for table `qc_cash_up`
...
ALTER TABLE qc_cash_up CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci;
ALTER TABLE qc_expense_categories CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci;
ALTER TABLE qc_expenses CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci;
...
UPDATE `qc_expenses`
  INNER JOIN `qc_suppliers`
    ON `qc_expenses`.`supplier_name` = `qc_suppliers`.`company_name`
SET `qc_expenses`.`supplier_id` = `qc_suppliers`.`person_id`;
...
ALTER TABLE `qc_expenses`
  ADD CONSTRAINT `qc_expenses_ibfk_3` FOREIGN KEY (`supplier_id`) REFERENCES `qc_suppliers` (`person_id`);
