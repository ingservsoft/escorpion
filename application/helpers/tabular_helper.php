...
function get_items_manage_table_headers()
{
	...
	$headers = array(
		...
		array('item_codigolocal' => $CI->lang->line('items_item_codigolocal')),
...
function get_item_data_row($item)
{
	...
	$columns = array (
		...
		'item_codigolocal' => $item->item_codigolocal,
		'name' => $item->name,
...
function get_expenses_manage_table_headers()
{
	...
	$headers = array(
		...
		/*array('caja' => $CI->lang->line('caja')),*/
		array('ingreso' => $CI->lang->line('ingreso')),
		array('egreso' => $CI->lang->line('egreso')),
		array('saldo' => $CI->lang->line('saldo'))
	);
...
*/
function get_expenses_data_row($expense, $saldoanterior)
{
	$CI =& get_instance();

	$controller_name = strtolower(get_class($CI));
	$valoringreso = $expense->tipo_movimiento=='Ingreso'?$expense->amount:NULL;
	$valoregreso = $expense->tipo_movimiento=='Egreso'?$expense->amount:NULL;
	return array (
		...
		'ingreso' => intval($valoringreso),
		'egreso' => intval($valoregreso),
		'caja' => $expense->caja,
		'saldo' => to_currency($saldoanterior+intval($valoringreso)-intval($valoregreso)),
		'edit' => anchor($controller_name."/view/$expense->expense_id", '<span class="glyphicon glyphicon-edit"></span>', array('class'=>'modal-dlg', 'data-btn-submit' => $CI->lang->line('common_submit'), 'title'=>$CI->lang->line($controller_name.'_update'))),
		'receipt' => anchor($controller_name."/receipt_expense/$expense->expense_id", '<span class="glyphicon glyphicon-print"></span>',	array( 'data-btn-submit' => "Imprimir", 'title'=>"imprimir")),
	);
}
...
*/
function get_expenses_data_last_row($expense,$saldoanterior)
{
	$CI =& get_instance();
	...
	$sum_tax_ingreso_expense = 0;
	$sum_tax_egreso_expense = 0;
	...
	foreach($expense->result() as $key=>$expense)
	{
		...
		if($expense->tipo_movimiento=='Ingreso')
		{
			$sum_tax_ingreso_expense += $expense->amount;			
		}
		else
		{
			$sum_tax_egreso_expense += $expense->amount;			
		}
	}
	...
	return array(
		...
		'ingreso' => '<b>'. to_currency($sum_tax_ingreso_expense).'</b>',
		'egreso' => '<b>'. to_currency($sum_tax_egreso_expense).'</b>'
	);
}
...
## cambios MARIO LARRAÑAGA nuevos modulos quierocancha ##
function get_canchas_manage_table_headers()
{
	$CI =& get_instance();

	$headers = array(
		array('id_cancha' => "ID"),
		array('urlfoto_cancha' => "Foto"),
		array('nombre_cancha' => "Nombre Cancha"),
		array('tipo_cancha' => "Tipo Cancha"),
		array('descripcion_cancha' => "Descripción"),		
		array('ids_bloquea' => "Bloquea"),
		array('tarifas'=>"Tarifas")
	);

	return transform_headers($headers);
}
function get_cancha_data_row($cancha)
{
	$CI =& get_instance();
	...
	return array (
		...
		'urlfoto_cancha' => '<img src="'.$cancha->urlfoto_cancha.'" width="120px" />',
		'nombre_cancha' => $cancha->nombre_cancha,
		'tipo_cancha' => $cancha->tipo_cancha,
		'descripcion_cancha' => $cancha->descripcion_cancha,		
		'ids_bloquea' => $cancha->ids_bloquea,
		
		'tarifas' => anchor($controller_name."/vertarifas/$cancha->id_cancha", '<span class="glyphicon glyphicon-th-list"></span>',	array('class'=>'', 'title'=>'Tarifas')),			
		
		'edit' => anchor($controller_name."/view/$cancha->id_cancha", '<span class="glyphicon glyphicon-edit"></span>',
			array('class'=>'modal-dlg', 'data-btn-submit' => $CI->lang->line('common_submit'), 'title'=>$CI->lang->line($controller_name.'_update')))
	);
}

function get_tarifas_manage_table_headers()
{
	$CI =& get_instance();
	...
	$headers = array(
		...
		array('lunes' => "Lunes"),
		array('martes' => "Martes"),
		array('miercoles' => "Miercoles"),		
		array('jueves' => "Jueves"),
		array('viernes'=>"Viernes"),
		array('sabado'=>"Sabado"),
		array('domingo'=>"Domingo")
	);

	return transform_headers($headers);
}
function get_tarifa_data_row($tarifa)
{
	$CI =& get_instance();
...
function get_convenios_manage_table_headers()
{
	$CI =& get_instance();

	$headers = array(
		array('convenio_id' => "ID"),
		array('cliente' => "Cliente"),
		array('tipoconvenio' => "Tipo Convenio"),
		array('descripcion' => "Descripcion"),
		array('periodopago' => "Periodo Pago (dias)"),		
		array('valorconvenio' => "Valor de Pago"),
		array('fecha_inicio'=>"Fecha Inicio"),
		array('fecha_fin'=>"Fecha Fin"),
		array('creadopor'=>"Creado Por")
	);

	return transform_headers($headers);
}

function get_convenio_data_row($convenio)
{
	$CI =& get_instance();
	$controller_name = strtolower(get_class($CI));
	...
	return array (
		...
		'periodopago' => $convenio->periodopago,		
		'valorconvenio' => $convenio->valorconvenio,
		'fecha_inicio' => $convenio->fecha_inicio,
		'fecha_fin' => $convenio->fecha_fin,		
		'creadopor' => $convenio->creadopor,		
		
		'edit' => anchor($controller_name."/view/$convenio->convenio_id", '<span class="glyphicon glyphicon-edit"></span>',
			array('class'=>'modal-dlg', 'data-btn-submit' => $CI->lang->line('common_submit'), 'title'=>$CI->lang->line($controller_name.'_update')))
	);
}
